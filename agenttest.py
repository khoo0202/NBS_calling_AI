...
async def entrypoint(ctx: JobContext):
    ...
    async def handle_transcribed(event):
        ...
        # Robust goodbye detection: ignore punctuation, match word boundaries
        goodbye_keywords = ["goodbye", "bye", "see you", "see ya", "farewell", "再见", "拜拜"]
        normalized = re.sub(r'[^\w\s]', '', text.lower())
        said_goodbye = any(re.search(r'\b' + re.escape(kw) + r'\b', normalized) for kw in goodbye_keywords)
        ...
        if caller_info.get("action") == "transfer_to_human":
            ...
            instruction += "How else may I assist you?"
            await session.generate_reply(instructions=instruction)
